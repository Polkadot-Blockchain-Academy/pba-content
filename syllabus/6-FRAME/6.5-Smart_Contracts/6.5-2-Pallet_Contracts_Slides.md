...
- WebAssembly is a standardized stack-based virtual machine
  - As oposed to a register based-machine as most existing real world architectures.
...
- WebAssembly is meant to be embedded by other applications as a way to express logic within these
applications. Such an application is called an "embedder". A programming language with a similar
use case is Lua.
- Each embedder provides the environment. The most important properties of this environment are:
  ...
  the environment (e.G `print_line`). The identifier of an import consists of a `module` and a `name`.
...
- The most prominent embedders are probably web browsers.
...
- wabt
- parity-wasm
- wasm-tools (wasm-parser, wasm-encoder, etc.)
- wasm-instrument
