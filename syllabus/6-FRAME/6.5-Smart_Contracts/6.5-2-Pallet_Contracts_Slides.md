...
```Rust
pub fn factorial(n: u64, out: &mut u64) -> u64 {
  if n == 0 {
    1
  } else {
    let result = n * factorial(n - 1, out);
    *out = result;
    return result;
  }
}
...
(module
  ...
  (func (param i64) (result i64)
    local.get 0       ;; load first parameter to stack
    i64.eqz           ;; load result of 'x == 0' to stack
    (if (result i64)   ;; blocks can return a result just like in rust
        ...
        i64.const 1 
        i64.sub       ;; n - 1
        call 0        ;; push result of factorial(n - 1)
        i64.mul       ;; return n * factorial(n - 1)
    ) 
  )
)
```
...
- This format is called 'wat' and is essentially a wasm assembler.
  ...
  - Parenthesis represent block and end_block instructions
...
- Entities within a Wasm code file (Wasm module) are enumrated by their appearence (starting with 0).
  - Calling the first defined function is done via `call 0` for example
...
```WebAssembly
local.get 1 ;; load val
local.get 0 ;; load address
i64.store 0 ;; store at address with offset 0
```
