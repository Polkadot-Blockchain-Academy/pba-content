...
<div style="font-size:calc(var(--size) / 2)">
...
TODO: replace with a figure of a mindful runtime and a mindless client. probably same figure as state opaque
...
struct ClientBlock<H: HeaderTrait> {
    header: H,
...
* Client does not know the internal extrinsic format. It can change!
...
* Find all the host functions and runtime apis that are defined in substrate!
...
* Going over the API, everything is encoded:
...
* At the abstraction level of `impl_runtime_apis!` or certain client code, this happens under the
  hood.
...
* Can the old host function be deleted from the client once we're done? 🤔
...
* 🧠 Same condition: client update first, old version kept FOREVER.
