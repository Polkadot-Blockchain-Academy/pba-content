...
generally the limits of the wasm. (TODO this can be a slide but probably already in other lessons)
...
- pov check: All is in wasm; the whole state machine and calls to host function directly call this runtime state machine, the db call from the state machine is simply the merkle proof). -> TODO can have a slide for it but probably already in other lessons.
...
Be clear that the size is limited for distribution question.

So out of the computational cost of runing the wasm logic there is the merkle proof computational cost.
...
(related to weigt v2: both computation and size factor).
...
TODO diagram binary trie proof against hex proof showing binary is smaller
