...
As focusing on the cumulus proof we will switch to the actual content of this lesson, ideas of the impact of using merkle trie for state and proof in regard to the Polkadot model.
...
This was already evoked in leg h before.
Just a reminder.
...
- block production: We run execute transaction from pool and actually part of execute block are run externally (on init, on finalize (on finalize contains next block storage root calculation call: the call to storage root!!!)).
...
Seeing that we can also conclude having less branch involve is better, so radix 16 choice is better than binary trie, but really these days size is a bigger concern.
...
Depending on your runtime logic chances are smaller proof will allow more computation in a block: by allowing more different data to be accessed and a bigger variety of extrinsic.
...
<img style="width: 400px; float: middle; margin-right: 50px;" src="../../../assets/img/5-Polkadot/5-3/proof2.svg"/>
...
<img style="width: 800px; float:middle; margin-right:50px" src="../../../assets/img/5-Polkadot/5-3/proof3.jpg"/>
