...
- Perhaps some additional information that would help block authors prioritize
...
Example: User-supplied length of a list that will be iterated over Number of calls within a batch
...
- Ethereum recently added [EIP1559](https://eips.ethereum.org/EIPS/eip-1559), which uses a fee + tip mechanism
...
In fact, this is often a bad design.

Block authors will want fees to go up, so may indulge in no-op transactions to boost fees.
