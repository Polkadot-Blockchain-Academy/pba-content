# Rust Code for Hashes

## Prelude

```rust
:sccache 1
:dep sp-core = { version = "6.0.0", git = "https://github.com/paritytech/substrate.git", branch = "polkadot-v0.9.25" }
rand = { version = "0.8.5", features = ["small_rng"] }
use sp_core::*;
use std::fmt::Debug;

/// Helper function to print slices of an array.
fn sized_print<H:Debug+?Sized>(h: &H) {
    println!("{:?}", h);
}

/// Helper function to compare a subset of bytes from an array.
fn sized_compare<H:std::cmp::PartialEq+?Sized>(a: &H, b: &H) -> bool {
	a == b
}
```

## Fixed-Size Outputs

```rust
let short_input_hash = blake2_256(&b"abcd"[..]);
let long_input_hash = blake2_256(&[0; 1024][..]);
assert_eq!(short_input_hash.len(), long_input_hash.len());
HexDisplay::from(&short_input_hash)
HexDisplay::from(&long_input_hash)
```

## Computation Speed Properties

```rust
use std::time::{Instant};
// Let's give the function 1kb of data to hash
let value_to_hash = [0; 1024];

// Blake2
let blake2_start = Instant::now();
for _ in 0..1000 {
	let _ = blake2_256(&value_to_hash[..]);
}
let blake2_elapsed_time = blake2_start.elapsed().as_micros();

// TwoX
let twox_start = Instant::now();
for _ in 0..1000 {
	let _ = twox_256(&value_to_hash[..]);
}
let twox_elapsed_time = twox_start.elapsed().as_micros();

// Compare times. We expect TwoX to be about 10x faster.
// Note this is native hardward. Wasm may be different.
println!("Time (us) for 1k rounds of Blake2: {:?}", blake2_elapsed_time);
println!("Time (us) for 1k rounds of TwoX:   {:?}", twox_elapsed_time);
```

## Pre-Image Attacks

```rust
use rand::prelude::*;

// We want to find some other pre-image that will give the same hash as "cambridge".
let attack_target = blake2_256(b"cambridge");

// Hopefully we could not actually succed in this! So let's cheat, and truncate the hash to just
// its first few bytes. We'll look for a collision on this truncated hash.
let difficulty = 2;
let mut count = 0u32;
loop {
	count += 1;
	// Generate some random array.
	let x: [u8; 16] = random();
	let x_hash = blake2_256(&x[..]);
	if sized_compare(&attack_target[0..difficulty], &x_hash[0..difficulty]) {
		println!("\nSecond pre-image found in {:?} attempts! {:?}", count, x);
		sized_print(&x_hash[0..difficulty]);
		sized_print(&attack_target[0..difficulty]);
		break;
	}

	// Let's not let our CPU run forever and bail.
	if count == 100_000 {
		println!("\nGiving up on pre-image attack");
		break;
	}
}
```

## Collisions

```rust
use rand::prelude::*;

// Here there is no "attack target". The attacker controls both inputs.
// TODO: Store previous hashes in an array and check for matches.

let difficulty = 2;
let mut count = 0u32;
loop {
	count += 1;

	// First value.
	let x: [u8; 16] = random();
	let x_hash = blake2_256(&x[..]);

	// Second value.
	let y: [u8; 16] = random();
	let y_hash = blake2_256(&y[..]);

	if sized_compare(&x_hash[0..difficulty], &y_hash[0..difficulty]) {
		println!("\nCollision found in {:?} attempts!", count);
		println!("x: {:?}", x);
		println!("y: {:?}", y);
		sized_print(&x_hash[0..difficulty]);
		sized_print(&y_hash[0..difficulty]);
		break;
	}

	if count == 100_000 {
		println!("\nGiving up on collision");
		break;
	}
}
```
