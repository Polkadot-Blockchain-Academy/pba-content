...
It should be difficult for someone to find two messages that hash to the same value (i.e. `H(x) == H(y)`).

**Difference from second pre-image attack:** In a second pre-image attack, the attacker only controls one input (presumably the other has already been signed). In a collision, the attacker controls both inputs but may attempt to trick someone into signing one message.
...
Keccak is used for Ethereum compatibility.
...
We often want to map one item to another, like an ID to a collection of information. For example, an asset ID to a struct containing asset details (e.g. some metadata, like name, symbol, and denomination).

We can use the hash of the ID (along with the module ID and storage item) to create the lookup key.

Keys can also be concatenated to form double and triple maps.
...
Modules within a system can have their own accounts controlled by their internal logic. The hash of some input (e.g. a byte-string representing the module) can be used as a system account.
...
- Users of a system may want to discuss proposals without storing the proposal on the system. Likewise, it should not be able to change the proposal without a new discussion.
...
Sometimes people want to store information in one place and reference it in another. For example, they may vote on executing some privileged instructions within the system.
...
Hash-based lists make the reference related to the data they are referencing. The properties of hash functions make them an obvious choice for this application.

Any change at any point in the list would have downstream changes to all hashes.
