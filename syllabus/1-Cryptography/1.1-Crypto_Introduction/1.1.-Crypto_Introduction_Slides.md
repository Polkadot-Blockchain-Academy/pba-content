...
Note: IMO we should not include this, so I've commented it out. To me, the "possession of some secret" is much more important than one-way-ness. E.g. with XOR, {E(t, key), D(t, key)} (encryption, decryption on some plain, cipher text, respectively) the functions E and D are the same, the one-way-ness comes from the fact that both require having agreement on a key, and without the key, performing D() is difficult (and E, meaningless).
...
Like physical signatures, cryptography may be used to give a reasonable expectation of a message's provenance (origin).
