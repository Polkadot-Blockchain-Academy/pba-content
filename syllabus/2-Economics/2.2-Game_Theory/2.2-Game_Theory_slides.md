...
TODO: formatting seems a bit off here.
...
Notes:
- You might now recognize these terms just yet, but next module we will dive deep into them.
- Blockchain space is economics on steroids, without trusting anybody, everything must be cryptographically and economically sound.
...
Notes:
- Game Theory was especially becoming prominent in the Cold War era, where the US and UDSSR analyzed every potential step of the other and calculated their **best response** (we will see later what this means).
- Auctions are an important factor in Polkadot, as you will see during this academy. The next lecture is dedicated to that from a game-theoretic perspective.
...
-  The game is static. For it to be dynamic, firms would need to make offers sequentially, _knowing what the firms before had offered_.
- Some people decide to make it dynamic by taking the offer back to their current employer or to another interview.
...
- Bonnie and Clyde are accused of robbing two banks.
  ...
  - The evidence for the second robbery is not sufficient and the two will only be convicted for that robbery in case of a confession by either or both.
...
Notes:
- k_1_star = 1/2 * [K - 1/2 * (K - k_1_star)]
- k_1_star = 1/2 * K - 1/4 * K + 1/4 * k_1_star
- k_1_star = 1/4 * K + 1/4 * k_1_star
- 3/4 * k_1_star = 1/4 * K
- k_1_star = K/3
