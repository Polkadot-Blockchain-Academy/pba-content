---
title: Price Discovery and Auctions
...
# Lecture 3: Price Finding Mechanisms: Auctions

Notes:

60 min + quick recap <= 20 min
...
# Recap: Results of the Experiments

<!-- TODO Insert QR code by which players can see the points they gained during the first day. That will be their endowment to bid in the NFT auctions-->
...
- The concepts of supply and demand help us understand price formation from a general perspective. They describe the _outcome_ of the price-finding process.
- However, they do not help much to understand _how exactly firms go about_ when setting the prices for the goods they sell.
- HOW are prices set in markets?

---

## Price Finding Methods
...
- The main question is: how does the firm _optimally_ sell its goods?
...
- Throughout, we assume that the buyer purchases the good if the price is lower than, or exactly equal to, her valuation. In the second case, the buyer is _indifferent_ between buying and not buying.
...
- However, for rarer goods whose sale does not happen that frequently, we often see a different mechanism: auctions.
- Auctions are also often used in a blockchain context: NFTs, parachain auctions, etc.

---

## Example Price Finding

<img style="width:600px" src="https://i.imgur.com/gLsgk4r.png" alt="Expected Rev"/>
...
<img style="width:600px" src="https://i.imgur.com/BUY6FfZ.png" alt="Expected Rev"/>

- Aside: prices, valuations, and revenue are given in the same currency unit, say, USD.
...
<img style="width:600px" src="https://i.imgur.com/yLLa8UL.png" alt="Expected Rev"/>
...
- There are two broad classes:
  - _Static Auction:_ all bidders submit their bids at the same time.
  - _Dynamic Auction:_ bidders submit bids over time.
...
- For this reason, static auctions are sometimes known as _sealed-bid auctions_, referring to a style where bids are submitted in sealed envelopes.
...
- Observe: It has a flavour of a second-price auction: the price you pay is decided by the second-to-last bidder to leave. In a sense the two auctions are _strategically equivalent._
...
- Observe: It has a flavor of a first-price auction: you pick the price you pay. In a sense the two are _strategically equivalent._
...
**Fact:** For each bidder, it is optimal to bid their actual valuation ($b_A=v_A$ and $b_B=v_B$) _no matter what the other bidder bids_.
...
- A player's action $\alpha$ is _weakly dominated_ by another action $\beta$ if $\beta$ produces an outcome that is at least as good as the outcome produced by $\alpha$, no matter what the other player does.
- A player's action $\alpha$ is _strictly dominated_ by another action $\beta$ if $\beta$ produces an outcome that is strictly better than the outcome produced by $\alpha$, no matter what the other player does.
...
<img style="width:600px" src="https://i.imgur.com/bx9RDse.png" alt="Expected Rev"/>

<!-- TODO DESIGN replace above -->
...
- The English auction is _strategically equivalent_ to the static second-price auction.
...
- One reason is that it is prone to _shill-bidding_.
...
**Fact:** In a first-price auction with two players, where the valuations are independent uniform values on $[0,1]$, it is a _Nash equilibrium_ for both bidders to bid half of their valuations, i.e., $b_A = v_A/2$ and $b_B=v_B/2$.
...
- An annoying problem for auctions on the blockchain is _front-running_.
...
# Workshop: The Candle Auction on Polkadot
...
<img style="width:600px" src="https://i.imgur.com/23Y8Cyv.png" alt="Expected Rev"/>
...
- That is, it is an instance of a _Package auction_.
