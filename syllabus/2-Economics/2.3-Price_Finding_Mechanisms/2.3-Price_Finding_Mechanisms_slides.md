...
- Gov't sells radio frequencies to broacasters.
...
- Valuations are _sampled unif. at random from $[0,1]$_.
- Valuations are independendent from one another.
- Each valuation is secret, but their distribution is public.
...
- What is her  _optimal price_ $p$? One maximizing her _expected revenue_.
...
- The simplest solution is for Selena to post a price publically, and hope that at least one of the buyers accepts it, as otherwise there is no sale. She needs to balance the possibility of low revenue if she posts a low price, and the possibility of not making a sale (and hence having zero revenue) is she posts a high price.
- We define the optimal price to be the one that maximizes the seller's *expected revenue*, i.e., the posted price times the probability of making a sale. Notice this is a sensible but strong assumption, as there can be other equally sensible notions of an optimal price for Selena. 
- The probability of making a sale is $(1-p^2)$. To see this, notice that if a buyer's valuation is sampled uniformly at random from interval $[0,1]$, then it will be below $p$ with probability $p$, and in this case the buyer rejects the price. And since the buyers' valuations are sampled independently, the probability that both buyers reject the price is $p\times p = p^2$. Conversely, the probability that at least one of them accepts the price is $(1-p^2)$. 
- Selena's revenue is $p$ in case of making a sale, and $0$ otherwise. Hence the expected revenue, expressed as a function of $p$, is $f(p)=p\times (1-p^2)$. For instance, for $p=0$, the probability of making a sale is 100% but the revenue is zero, while for $p=1$, the revenue in case of a sale would be $1$ but the probability of making a sale is zero. Hence the optimal price $p$ is neither $0$ nor $1$, but some value in between that maximizes $f(p)$. 
...
- In this lecture we will have some fun analyzing the bidders' best strategies as well as the auctioneer's best strategies.
...
- There are two broad classes:
  - _Static Auctions:_ bidders submit their bids at the same time.
  - _Dynamic Auctions:_ bidders submit bids over time.
- The difference is whether or not bidders 
can react to the bids of others and adjust their own.
...
- Static auctions are also called seald-bid auctions, as many times in practice the bids are submitted in sealed envelopes to the auctioneer.
...
- To analyze the bidders' best strategy, we assume that each bidder wants to maximize their expected profit. Again, this is a sensible but strong assumption, as they could have some other equally sensible objectives. By the end of the lecture we will highlight how this assumption can be a bit unrealistic in practice.

---

## Second-price auction

**Truthful Equilibrium:** It is a _dominant strategy_ to bid truthfully.

*Dominant strategy:* A strategy whose outcome is at least as good 
as the outcome of any other strategy, _no matter what the other bidder bids_.

*Proof sketch:* Imagine you are Alice (same proof for Bob)

* |---------- your valuation ---------- Bob's bid ----------|

* |---------- Bob's bid ---------- your valuation ----------|
...
- In the case that Bob's bid is higer than your valuation, bidding truthfully or underbidding both lead to losing and having a revenue of zero. 
...
- The proof can be easily generalized to any number of bidders (to obtain that in a second-price auction, it is a dominang strategy for each bidder to bid truthfully.
...
- This is not quite as good as posting a price
  (which provided expected revenue $0.38$). Why not?
...
**Fact:** Under any reserve price $r$, it is still optimal to bid truthfully, 
and if Selena sets $r=0.5$, her expected revenue is $\approx 0.42$, 
so it is better than posted price (where expected revenue was $\approx 0.38$).
...
- Second-price and English auctions popular among theorists,
  not so popular among practitioners.
- One reason is that they are prone to _shill-bidding_:
bidder that acts on behalf of the auctioneer to drive up the price.
...
**Equilibrium strategy:** It is a _Nash equilibrium_ 
for each bidder to bid **half** their own valuation.

_Nash equilibrium:_ A set of stategies, one per player, 
where no one has an incentive to change their strategy.

Notes:

- A Nash equilibrium, is a central concept in game theory, as we already saw yesterday.
...
**Proof idea:** suppose you are Alice
...
- Unfortunately, the full proof is outside the scope of the presentation, but we provide it here in the lecture notes:
...
- In summary, if Bob bids half his valuation, it is optimal for Alic to bid has her valuation, and vice versa. Together we have a Nash equilibrium.
...
- Is there a deeper connexion going on here? Or is it just a coincidence that the expected revenues are all the same so far?
...
_Answer_: In a dynamic auction, a bidder can use 
the bids of others as additional signals of the correct valuation. 
If bids so far seem high, my initial estimate must be low, and vice versa, 
so I can adjust my personal estimate. Hence estimates converge. 

In a static auction, there is no convergence of estimates, 
so it is more likely that some bidders keep unusually high estimates. 
As a result, there is a higher chance that the winner 
ends up paying more than the correct valuation. 
...
It is observed in practice that first-price auctions 
lead to higher revenue than second-price auctions.

This _violates_ the equivalence of revenues,
so an assumption in our analysis fails consistently.
...
_Answer:_ **Risk aversion.** People prefer lower uncertainty games, 
even if this means lower expected profits. 

_Would you rather win a million dollars with a 50% chance, 
or 300 thousand with a 90% chance?_

In Nash equilibrium analysis for first-price auctions, 
we claimed that if Bob bids half his valuation, 
then Alice should bid half hers, so each wins 50% of time. 
...
- This is something important to remember. If you want to maximize the auctioneer's revenue and don't mind playing psichological games with the bidders, you should choose first price auctions over second price auctions.

---

## Front Running

Computer systems may have _front runners_: 
special nodes can see an incoming bid, 
react by creating their own bid, 
and make it look like their bid was created first. 

If you run an auction on a system with front runners, 
which of the four auctions would you use? Why?
...
In a Dutch auction, if Fred is a bidder 
he waits for first bidder to signal accepting the price, 
and Frend makes the signal first. 
He's guaranteed to win with least possible price.

In second-price auction, if Fred is auctioneer 
he can shill bid successfully: 
when a highest bid arrives, he creates bid slightly under it 
and makes it appear as if it was created first.
...
In a first-price auction, if Fred is bidder 
and if he can "open the seals" 
he can win by slightly outbidding highest bid.
...
In a dynamic auction with a fixed bidding time window, 
 _sniping_ is placing a highest bid as late as possible, 
 so other bidders can't react and overbid you. 
 The practice leads to low revenue.
 Particularly bad when done by a front runner (mircroseconds).
...
- Sniping protection: the longer you wait to bid, 
the higher the chances the auction ends.
