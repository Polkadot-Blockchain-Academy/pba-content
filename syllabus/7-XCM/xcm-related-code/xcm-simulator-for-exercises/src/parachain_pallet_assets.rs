...
use xcm_builder::{
	AccountId32Aliases, AllowUnpaidExecutionFrom, CurrencyAdapter as XcmCurrencyAdapter, FungiblesAdapter,
	EnsureXcmOrigin, FixedWeightBounds, FixedRateOfFungible, IsConcrete, LocationInverter,
	NativeAsset, SignedAccountId32AsNative,
	SignedToAccountId32
};
use xcm_executor::{Config, XcmExecutor, traits::{MatchesFungibles, Error as MatchError}};
...
pub struct TokenAssetMatcher;
impl MatchesFungibles<u128, Balance> for TokenAssetMatcher {
	fn matches_fungibles(a: &MultiAsset) -> result::Result<(u128, Balance), MatchError> {
		match a {
			MultiAsset { id: Concrete(location), fun: Fungible(amount) } => {
				if location == &MultiLocation::parent() {
					return Ok((1u128, *amount))
				}
				else {
