...
### _Cross-chain Message Passing (XCMP)_
...
<img style="width: 600px;" src="../../../assets/img/7-XCM/xcm-stack.png" alt="XCM stack"/>
...
For chains that **trust** each other, teleporting is possible.

_Destroying_ sender-side Assets **AND** _minting_ the equivalent Assets on the receiving side.

</widget-column>
<widget-column>

```rust
// XCM teleported (most of) 1 DOT from the
// Polkadot Relay Chain to its sovereign
// account on Statemint. We’ll assume that
// the fees are already paid on the
// Polkadot side.
...
If the receiver cannot enforce sender to destroy the asset, the sending chain really has no basis for minting the asset on the back of a message.
