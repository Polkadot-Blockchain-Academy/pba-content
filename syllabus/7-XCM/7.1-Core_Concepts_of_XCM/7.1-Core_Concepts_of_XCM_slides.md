...
3. Enables single use-case chains e.g. Statemint/e as asset parachains
...
We need a way to reason about, and pay for, any required fees on the reciving CS.
...
This will be very powerful latter on (Origins)
...
<!-- TODO: Move this slide to the next lesson -->
...
- Instruction
    - Instructions that deal with local assets only (`WithdrawAsset`, `DepositAsset`)
    - Instructions that prepare local assets to be transferred across consensus systems (`InitiateTeleport`, `InitiateReserveWithdrawal`)
    - Instructions that interact with other configurable parameters of the XCVM (`ClaimAsset`, `SubscribeVersion`)
- Trusted Indication
- Information

Notes:
`Instruction` is a bad name for the kind of XCVM instructions that we have, but it means instructions that result in a state change in the local consensus system, or instruct the local consensus system to achieve some desired behavior such as reporting its state.
...
<!-- TODO: Instructions Deep Dive, split into slides, and move to next lesson -->
...
Notes:
This is what ties XCM and XCMP together.
