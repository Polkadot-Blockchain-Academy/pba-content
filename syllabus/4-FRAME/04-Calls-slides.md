...
The terms "call", "extrinsic", and "dispatchable" all get mixed together.
...
Every pallet call must have an `origin` parameter, which uses the `OriginFor<T>` type from `frame_system`.

```rust
/// Type alias for the `RuntimeOrigin` associated type of system config.
...
pub trait Parameter: Codec + DecodeWithMemTracking + EncodeLike + Clone + Eq + fmt::Debug + scale_info::TypeInfo {}
impl<T> Parameter for T where T: Codec + DecodeWithMemTracking + EncodeLike + Clone + Eq + fmt::Debug + scale_info::TypeInfo {}
```
...
Pallet Index comes from the `#[runtime::pallet_index]` in the runtime definition. If indices change without explicit labeling, a transaction generated by a wallet (like a Ledger) could be incorrect!
