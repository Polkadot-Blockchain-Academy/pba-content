...
`010101` = 2<sup>1</sup> + 2<sup>3</sup> + 2<sup>5</sup> = 2 + 8 + 32 = 42
...
It can be confusing to read the output, and keep in mind endian-ness.

The order of bytes in the vector follow endian-ness, but the hex of each byte is the same, and independent of endian-ness.
...
* 42 as binary: `101010` = `0x2a`.
* Add `00` to the LSB.
* `10101000` = `0xa8`: 168 as decimal.
...
* 69 as binary: `1000101` = `0x45`.
* Add `01` to the LSB.
* `100010101` = `0x1501`: 277 as decimal.
...
* 65535 as binary: `1111111111111111` = `0xffff`.
* Add `10` to the LSB.
* `111111111111111110` = `0xfeff0300`: 262142 as decimal.
