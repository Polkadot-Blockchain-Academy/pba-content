...
- after the "prevote" phase the "estimate" can only move backwards
...
The round is deemed "completable" when the round's "estimate" has been finalized, and validators start a new round
...
If the set of pre-votes extends the last finalized chain, then each validator will cast a “precommit” to that chain.

Each validator waits to receive enough precommits to form a commit message on the newly finalized chain.
