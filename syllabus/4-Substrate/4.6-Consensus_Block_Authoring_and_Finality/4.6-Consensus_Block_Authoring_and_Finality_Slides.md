...
<!-- TODO DESIGN - Add diagram showing different forks with different number of primary slot blocks -->
...
<!-- TODO CONTENT: cc @SME - Not sure where to leave this slide - perhaps this can be followed by a quick exercise demonstrating manual and instant seal in action? -->
...
**Ensures block finality**: as soon as more than $\frac{2}{3}$ of validators attest to a chain containing a certain block, all blocks leading up to that one are finalized at once.
...
See the original paper for GRANDPA: https://arxiv.org/pdf/2007.01560.pdf

---

## A GRANDPA round

<img style="width: 650px" src="../../../assets/img/4-Substrate/4.6-validators-vote-on-chain.png"/>
...
If $\frac{2}{3}$ of validators are honest, this block should extend the chain that the primary broadcasts.
This new chain could be several blocks longer than the last finalized chain.
...
<img style="width: 800px" src="../../../assets/img/4-Substrate/4.6-fork-choice.png"/>
...
<!-- TODO DESIGN: add a diagram showing multiple pending validator set changes -->
...
Introduce GRANDPA's fork choice rule.

Source of image: https://wiki.polkadot.network/docs/learn-consensus#fork-choice
...
An actor with unlimited resources could potentially build a competing chain and expend enough computational resources to create a chain that did not contain a specific block.
...
This implies that at least $\frac{1}{3}$ of the validators voted on these two chains.
...
1. **Why didn’t you consider a block final, as you voted to finalize a second block?**<br>
...
1. **Which pre-votes for the first round have you seen?**<br>
   Honest validators report other validators and the votes they received from peers.<br>
   The union of both sets exposes validators who voted for two conflicting chains.

While the _client_ only runs the GRANDPA algorithm, the _runtime_ defines the rules for punishing faulty validators (i.e. when $>\frac{1}{3}$ of the validators have equivocated).

Notes:

2 is assuming 1 is answered.

Specifications for punishing nodes is protocol-specific - Substrates doesn't care - it's in the chain’s business logic, not in the consensus layer.
...
<!-- TODO CONTENT: cc @SME Andre - not sure where we want to go here and in following slides. Please add more content. -->

---

## How do we decide who the authorities are?

<!-- TODO CONTENT: the authorities are dictated by the runtime (usually through staking). include information about the interaction between the runtime and consensus code, namely that it reads this information through digest data added to the block -->

---

## Finality Stalls

What happens when more than $\frac{1}{3}$ are offline ?
...
On "different failure bounds": With BFT agreement (GRANDPA) you need $\frac{2}{3}$ of the validators to be working in order to make progress, if $\frac{1}{3}$ of the validators go offline the system will halt.
