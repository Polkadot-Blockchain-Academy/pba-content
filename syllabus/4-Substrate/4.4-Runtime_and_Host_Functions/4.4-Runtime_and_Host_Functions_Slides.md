...
Instructor: Bastian Köcher, Joshy Orndorff
...
![Diagram of Substrate Node](../../../img/4-Substrate/4.4-node-diagram)

* Inside the boundary, in wasm, is the runtime
* Outside the boundary, in Rust, is the client

<!-- I got this syntax from https://revealjs.com/speaker-view/ is this the right way? -->
<aside class="notes">
...
* Construct Blocks -- not all nodes do this
* Consensus-adjascent tasks like authority set management
...
1. General software best practices. Separation of concerns. Most chains will only want to customize the Runtime.
...
Analogy you can flash a new program on to your microcontroller. It can even use the same data that is stored on disk and same peripheral hardware. Likewise, you can deploy a new runtime to the blockchain.
...
<div class = "right">
  ...
  ![Diagram of Substrate Node with RuntimeAPIs and Host Functions highlighted](../../../img/4-Substrate/4.4-node-with-apis-labeled.png)
...
![Two way Communication between Client and Runtime](../../../img/4-Substrate/4.4-calling-between-client-and-runtime.png)
...
Exercise: Write one that acts as an adder and substracter. Add more operations as well like multiply, mod, whatever you want. This introduces the idea of transaction types.
...
- Accessors - There aren't many standard accessor methods, but anyone can write their own.
...
First answer: You dont *have* to make the round trip. There is the StorageBackend. (Frontier occasionally uses this _as an optimization_). But you should make the trip because...
...
Did you ever wonder why the transacion pool logic is in the runtime? The TxPool is offchain. It is not part of the STF. If the runtime is the STF, why wrap TxPool logic into it.
...
TODO @basti do you have something in mind here Can I help? IMO just saying the two points above is enough
...
@Basti I don't think "non-consensus code path" is right. The txpool runtime api is non-consensus code-path but still needs to be on-chain.
