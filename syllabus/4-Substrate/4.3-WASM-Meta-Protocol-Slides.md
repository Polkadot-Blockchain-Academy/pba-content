...
- In some sense, this lecture is a deeper dive into this decision, and its _tradeoffs_.
...
Ideally, a client should only use the metadata to find the answer to these unknown variables.
...
Also, it is arguable to say that the runtime is the boss here. The client must server it fully, but the runtime may or may not want to support certain APIs for certain applications.
